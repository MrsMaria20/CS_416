...
public class BinarySearchTree {
    ...
    private Data find(Node n, String key) {
        // If node is null, the key doesn't exist
        if (n == null) {
            return null;
        }
        
        // Compare the key we want with this node's key
    ...
    private Data maxKey(Node n) {
        // If node is null, there's no data to return
        if (n == null) {
            return null;
        }
        
        // If no right child, this node has the max key
        if (n.right == null) {
            return n.data;
        }
        
        // Right child has a larger key, so keep going right
    ...
    private Data maxValue(Node n) {
        // If node is null, there's no value to return
        if (n == null) {
            return null;
        }
        
        // Start with the current node's value as the max
        Data max = n.data;
        
        // Check the left subtree for the maximum value
        Data leftMax = maxValue(n.left);
        if (leftMax != null && leftMax.value > max.value) {
            max = leftMax;
        }
        
        // Check the right subtree for the maximum value
